...
SELECT {source_id} AS SourceDataServiceID, '{network}' AS Network, sc.SiteCode, sc.SiteName, s.Latitude, s.Longitude, s.State, 
s.County, s.SiteType,
...
class SqlSnippets(object):
    DB_SQL_MAP = {
        'envirodiy': lambda name, db: EnvirodiyDataSeriesSnippets(name, db),
        'iUTAH_Logan_OD': lambda name, db: iUtahSqlSnippets(name, db),
        'iUTAH_RedButte_OD': lambda name, db: iUtahSqlSnippets(name, db),
        'iUTAH_Provo_OD': lambda name, db: iUtahSqlSnippets(name, db),
        'iUTAH_GreenMeadows_OD': lambda name, db: iUtahSqlSnippets(name, db),
        'iUTAH_NorthwestField_OD': lambda name, db: iUtahSqlSnippets(name, db),
        'iUTAH_UtahLake_OD': lambda name, db: iUtahSqlSnippets(name, db)
    }

    def __init__(self, name, database, language, compile_dataseries, purge_catalog, reset_sequence, get_sites,
                 get_datavalues):
        self.name = name
    ...
    @staticmethod
    def GetSqlSnippets(credentials):
        """
        :rtype: SqlSnippets
        """
        return SqlSnippets.DB_SQL_MAP[credentials.name](credentials.name, credentials.tsa_catalog_source['database'])


class EnvirodiyDataSeriesSnippets(SqlSnippets):
    def __init__(self, name, source_database):
        SqlSnippets.__init__(self, name, source_database, 'postgresql', envirodiy_catalog_script, catalog_purge,
                             sequence_reset, get_sites_envirodiy, envirodiy_get_datavalues)
...
class iUTAH_MAPS:
    NETWORKS = {
        ...
        'iUTAH_NorthwestField_OD': 'Northwest Field Canal',
        'iUTAH_GreenMeadows_OD': 'Green Infrastructure Research',
        'iUTAH_UtahLake_OD': 'Utah Lake'
    }
    ...
    WOF = {
        ...
        'iUTAH_GreenMeadows_OD': 'greenmeadowswof',
        'iUTAH_NorthwestField_OD': 'NorthwestFieldWOF',
        'iUTAH_UtahLake_OD': 'UtahLakeWOF'
    }
    ...
    SourceID = {
        ...
        'iUTAH_RedButte_OD': 2,
        'iUTAH_Provo_OD': 3,
        'iUTAH_GreenMeadows_OD': 7,
        'iUTAH_NorthwestField_OD': 8,
        'iUTAH_UtahLake_OD': 9
    }
...
class iUtahSqlSnippets(SqlSnippets):
    def __init__(self, name, source_database):
        fetch_script = iutah_catalog_script.format(DATABASE=source_database, network=iUTAH_MAPS.NETWORKS[name],
                                                   wof=iUTAH_MAPS.WOF[name], source_id=iUTAH_MAPS.SourceID[name])
        get_sites = get_sites_iutah.format(network=iUTAH_MAPS.NETWORKS[name])
        SqlSnippets.__init__(self, name, source_database, 'mssql', fetch_script, catalog_purge, sequence_reset,
                             get_sites, iutah_get_datavalues)
        self.extract_identifying_args = lambda dataseries, index: {
